...
// Optional database provider hint for ORM targets (Prisma datasource provider,
// Sequelize dialect). Defaults to PostgreSQL when omitted.
export type ModelDbProvider =
  | "postgresql"
  | "mysql"
  | "sqlite"
  | "mssql"
...
// Note: Prisma doesn't have a built-in Oracle provider — there's only an
// experimental community plugin. We emit the postgresql provider with a
// banner comment so the generated file at least scaffolds correctly and
// the user knows to swap in the right provider when they pick a driver.
const PRISMA_PROVIDER: Record<ModelDbProvider, string> = {
...
const SEQUELIZE_DIALECT: Record<ModelDbProvider, string> = {
  postgresql: "postgres",
  mysql: "mysql",
  sqlite: "sqlite",
  mssql: "mssql",
...
function generatePrismaSchema(
  ...
  const oracleNote =
    provider === "oracle"
      ? `// NOTE: Prisma has no first-party Oracle datasource provider. We
// fall back to "postgresql" so this file at least scaffolds. To run
// against Oracle, replace the provider with the connector you choose
// (e.g. the community oracle preview, or generate Sequelize models
// instead via Super Schema's Models tab → Sequelize).
`
      : "";
...
function generateSequelizeFile(
  schema: Schema,
  provider: ModelDbProvider
): string {
  const driverHint =
    provider === "mssql"
      ...
      : provider === "oracle"
        ...
        : "";
  const header = `// Generated by Super Schema
// Sequelize models — npm i sequelize${driverHint}

import { Sequelize, DataTypes } from 'sequelize';

export const sequelize = new Sequelize(process.env.DATABASE_URL!, {
  dialect: '${SEQUELIZE_DIALECT[provider]}',
  logging: false,
});
`;
...
export function generateModels(
  schema: Schema,
  target: ModelTarget,
  provider: ModelDbProvider = "postgresql"
): string {
  ...
  switch (target) {
    case "prisma":
      return generatePrismaSchema(schema, provider);
    case "sequelize":
      return generateSequelizeFile(schema, provider);
    case "dbml":
